...
router.get('/', authMiddleware, authorize('SECRETARY', 'ADMIN'), questionsController.getAll)
router.patch('/:id', authMiddleware, authorize('SECRETARY', 'ADMIN'), questionsController.updateStatus)
```
...
**`create`** — registra a pergunta no banco com status inicial `OPEN`.
...
interface CreateQuestionDto {
  text: string     // texto da dúvida
  email: string    // e-mail institucional para resposta
}
...
Preenche QuestionForm → POST /questions { text, email }
        ↓
Pergunta criada com status OPEN → 201
        ↓
Secretária acessa painel → GET /questions?status=OPEN
        ↓
Secretária responde o aluno por e-mail (fora do sistema)
        ↓
Secretária atualiza status → PATCH /questions/:id { status: 'ANSWERED' }
        ↓
Pergunta marcada como ANSWERED → 200
```
...
- O status inicial de toda pergunta criada é sempre `OPEN` — nunca aceite status no body do `POST`
...
- Novos status além de `OPEN` e `ANSWERED` exigem alinhamento com o PO e atualização do enum no schema Prisma
