...
```
Requisição → ProtectedRoute (valida autenticação) → RoleGuard('SECRETARY') → Página
```
...
pelos alunos via chatbot (RF05), com filtro por status (`em aberto`, `respondida`).
...
- Componentes de página **importam apenas** de `features/secretary/` e `components/layout/`
