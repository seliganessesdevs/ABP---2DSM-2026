...
```
utils/
├── hash.ts       # Gera e compara hashes de senha com bcrypt
...
Abstrai o `bcrypt` em duas funções simples. Senhas **nunca** são armazenadas em texto puro — sempre passam por aqui antes de ir ao banco.
...
O custo do bcrypt (`saltRounds`) é configurado via `env.ts` — não hardcoded aqui. Aumentar o custo aumenta a segurança mas também o tempo de resposta do login.
...
E com isso, todos os READMEs internos estão cobertos. A sequência completa ficou:

| Pasta | Responsabilidade |
|---|---|
| `config/` | Infraestrutura — env e banco |
| `middlewares/` | Interceptadores globais de requisição |
| `modules/` | Domínios de negócio |
| `errors/` | Erros controlados |
| `utils/` | Funções puras reutilizáveis |

Quer que eu escreva agora o **README raiz** do projeto também?